...
class VectorIngestionPipeline:
    def __init__(self, config_path="config/config.yaml"):
        ...
        '''
        db_path = os.path.join(self.base_dir, "qdrant_db")
    ...
    def setup_qdrant(self):
        """
            Qdrant prepares the collection (or reinstalls it if it already exists).
        """
        if self.client.collection_exists(self.collection_name):
            self.client.delete_collection(self.collection_name)
            print(f"The old collection has been deleted: '{self.collection_name}'")
    ...
    def run_pipeline(self, limit=None):
        ...
        if limit:
            print(f"Only the first {limit} row is taken for testing.")
            df = df.head(limit)
        ...
        batch_size = 64
        print(f"Vectorization and Qdrant loading are starting (Batch Size: {batch_size})...")

        
        for i in tqdm(range(0, len(documents), batch_size), desc="Upload to the Qdrant"):
        ...
        print(f"\n  SUCCESS! A total of {len(documents)} items have been loaded into Qdrant.")
